    ...
    try
        while true
            ...
            for task in Iterators.partition(eachindex(parsing_ctx.eols), task_size)
                ...
                _parse_rows_forloop!(result_buf, view(parsing_ctx.eols, task_start:task_end), parsing_ctx.bytes, parsing_ctx.schema, options, parsing_ctx.comment, parsing_ctx._eager_parsing)
